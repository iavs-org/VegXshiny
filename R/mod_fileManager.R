      ...
      fluidRow(
        column(
          ...
          tagList(       
            ...
            mod_fileBrowser_ui(ns("fileBrowser")),
            tags$hr(),
            div(
...
mod_fileManager_server <- function(id, action_log, log_path){
  moduleServer( id, function(input, output, session){
              ...
              showModal(
                modalDialog(
                  selectizeInput(ns("excel_sheets_selected"), 
                                 ...
                                 selected = excel_sheets_available,
                                 multiple = T, 
                                 width = "100%"),
                  footer = tagList(
    ...
    #### File browser ####
    mod_fileBrowser_server("fileBrowser", user_data)
        ...
        div(
          ...
          fluidRow(
            column(width = 12,
                   ...
                   actionButton(ns("delete_file"), "Delete selected file", width = "130px", class = "btn-xs")
            )
          ),
    ...
    observeEvent(eventExpr = input$confirm_save,
                 handlerExpr = {
                   tryCatch({
                     ...
                     insertUI(selector = paste0("#", ns("edit_toolbar")),
                              where = "afterEnd",
                              ui = actionButton(ns("delete_file"), "Delete selected file", width = "130px", class = "btn-xs"))
