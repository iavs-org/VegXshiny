...
#' @param namespaces A named list with xsd definitions (see global.R)
#' 
...
simplify_vegx_node = function(node, ns, namespaces){
  ...
  } else if(xml_name(node) %in% c("complexType", "sequence")){    # Not a leaf: Skip/Remove containers (sequences, complexTypes)
